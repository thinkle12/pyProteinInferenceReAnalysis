...
def plotPrestReport(protFile, vial):
  ...
  print("#significant protein groups Reported 5% FDR: {}".format(numSignificant))
  print("#significant protein groups Observed 5% FDR: {}".format(sum(1 if qval < 0.05 else 0 for qval in entrapmentFDRs)))
  print("Observed FDR at reported 5% FDR: {}".format(entrapmentFDRs[numSignificant-1]))
  numRows, numCols = 2, 2
  ...
  summary_text = (
    f"# significant protein groups Reported 5% FDR: {numSignificant}\n"
    f"# significant protein groups Observed 5% FDR: {sum(1 if qval < 0.05 else 0 for qval in entrapmentFDRs)}\n"
    f"Observed FDR at reported 5% FDR: {entrapmentFDRs[numSignificant - 1]:.4f}"
  )

  # Add the text to the figure
  plt.figtext(
    0.5, 0.01,  # Position: x=0.5 (center), y=0.01 (near bottom)
    summary_text,
    wrap=True,
  ...
  plot_file = 'plots/' + os.path.basename(protFile).split(".")[0] + 'png'
